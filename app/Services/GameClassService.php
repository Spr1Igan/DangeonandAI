...
class GameClassService
{
    ...
    public function createDraft(User $user, array $input): GameClass
    {
        ...
        return DB::transaction(function () use ($user, $name, $data) {
            $entry = $user->gameClasses()->create(['name' => $name]);
    ...
    public function updateDraft(User $user, int $entryId, int $versionId, array $input): ClassVersion
    {
        $data = $this->versionData($input);

        return DB::transaction(function () use ($user, $entryId, $versionId, $data) {
            $entry = $user->gameClasses()->lockForUpdate()->findOrFail($entryId);
            $this->ensureAvailable($entry);
            $version = $entry->versions()->lockForUpdate()->findOrFail($versionId);

            if ($version->status !== 'draft') {
                throw ValidationException::withMessages([
                    'class_version' => 'Редактировать можно только черновик. Создай новую версию.',
                ]);
            }
    ...
    public function finalize(User $user, int $entryId, int $versionId): ClassVersion
    {
    ...
    public function createVersion(User $user, int $entryId, int $sourceVersionId): ClassVersion
    {
        return DB::transaction(function () use ($user, $entryId, $sourceVersionId) {
            ...
            if ($source->status !== 'final') {
                throw ValidationException::withMessages([
                    'class_version' => 'Новую версию создаём на основе утверждённой.',
                ]);
            }

            if ($entry->versions()->where('status', 'draft')->exists()) {
                throw ValidationException::withMessages([
                    'class_version' => 'Уже есть черновик. Сначала заверши его.',
                ]);
            }
    ...
    public function archive(User $user, int $entryId): GameClass
    {
    ...
    public function restore(User $user, int $entryId): GameClass
    {
        return DB::transaction(function () use ($user, $entryId) {
            $entry = $user->gameClasses()->lockForUpdate()->findOrFail($entryId);
    ...
    private function ensureAvailable(GameClass $entry): void
    {
        if ($entry->status === 'archived') {
            throw ValidationException::withMessages([
                'class_version' => 'Сначала восстанови запись из архива.',
            ]);
        }
    }
}
